...
class BaseFrame(tk.Frame):
    ...
    def __init__(self, parent, root):
        ...
        self.keypointOptions = tk.OptionMenu(*((self, self.keypointTypeVar) + tuple(x[0] for x in keypointClasses)))
        ...
        self.descriptorOptions = tk.OptionMenu(
            *((self, self.descriptorTypeVar) + tuple(x[0] for x in descriptorClasses)))
        ...
        self.matcherOptions = tk.OptionMenu(*((self, self.matcherTypeVar) + tuple(x[0] for x in matcherClasses)))
...
class KeypointDetectionFrame(BaseFrame):
    ...
    def computeKeypoints(self, *args):
        if self.image is not None:
            ...
            self.setStatus('keypoints computed!')
            self.drawKeypoints()
        else:
    ...
    def drawKeypoints(self, val=None):
        ...
        self.setStatus('threshold=' + str(threshold))
        if self.image is not None and self.keypoints is not None:
            kps = self.thresholdKeyPoints(self.keypoints, threshold)
            self.setStatus('afterthreshold=' + str(len(kps)))
            img = self.image.copy()
            cv2.drawKeypoints(self.image, kps, img, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS, color=RED)
            self.setStatus('kp image created')
            self.imageCanvas.drawCVImage(img)
            self.setStatus('Found ' + str(len(kps)) +
                           ' keypoints')
        else:
            self.setStatus('No keypoints!')
...
if __name__ == '__main__':
    ...
    root.title('Xidian CS5201 - Feature Detection Project')
